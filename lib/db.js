...
export async function createOAuthToken(token) {
  const result = await pool.query(
    `INSERT INTO oauth_tokens (id, "platformKey", provider, "accessToken", "refreshToken", "tokenType", "expiresAt", scopes, metadata, "isActive", "createdAt", "updatedAt")
...
export async function getOAuthTokensByPlatform(platformKey) {
  const result = await pool.query(
    `SELECT * FROM oauth_tokens WHERE "platformKey" = $1 AND "isActive" = true ORDER BY "createdAt" DESC`,
    [platformKey]
  );
  return result.rows;
}

export async function getActiveOAuthToken(platformKey) {
  const result = await pool.query(
    `SELECT * FROM oauth_tokens WHERE "platformKey" = $1 AND "isActive" = true ORDER BY "createdAt" DESC LIMIT 1`,
    [platformKey]
  );
  return result.rows[0] || null;
}
