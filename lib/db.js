...
export async function createAccessItem(agencyPlatformId, item) {
  const result = await pool.query(
    `INSERT INTO access_items (
      ...
      "validationMethod", "createdAt", "updatedAt"
    ) VALUES (
...
export async function getAllIntegrationIdentities() {
  const result = await pool.query(`SELECT * FROM integration_identities ORDER BY name`);
  return result.rows;
}

export async function getIntegrationIdentityById(id) {
  const result = await pool.query(`SELECT * FROM integration_identities WHERE id = $1`, [id]);
  return result.rows[0] || null;
}

export async function createIntegrationIdentity(identity) {
  const result = await pool.query(
    `INSERT INTO integration_identities (id, name, type, identifier, description, "isActive", metadata, "createdAt", "updatedAt")
