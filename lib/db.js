...
export async function syncPluginsToCatalog(manifests) {
  ...
  // Get all existing slugs and names
  const existing = await pool.query(`SELECT slug, name FROM catalog_platforms`);
  const existingSlugs = new Set(existing.rows.map(r => r.slug));
  const existingNames = new Set(existing.rows.map(r => r.name));

  for (const m of manifests) {
    const slug = m.platformKey;
    // Skip if slug OR name already exists in DB
    if (existingSlugs.has(slug) || existingNames.has(m.displayName)) continue;
    ...
    await pool.query(
      ...
      [
        ...
        m.icon || 'fas fa-plug',
        m.tier || 2,
        m.clientFacing !== false,
        m.automationCapabilities?.automatedProvisioningSupported ? 'High' : 'Medium',
        supportedItemTypes,
        JSON.stringify(accessPatterns),
        oauthSupported
      ]
    );
    console.log(`[Catalog Sync] Auto-discovered plugin "${m.displayName}" → inserted into catalog`);
  }
}
