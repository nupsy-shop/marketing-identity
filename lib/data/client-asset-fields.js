...
export function getClientAssetFields(platformName, itemType = 'NAMED_INVITE') {
  ...
  if (ASSET_FIELDS_BY_PLATFORM[platformKey]) {
    const platformFields = ASSET_FIELDS_BY_PLATFORM[platformKey];
  ...
  // Try more specific matches first (longer keys are more specific)
  const sortedKeys = Object.keys(ASSET_FIELDS_BY_PLATFORM).sort((a, b) => b.length - a.length);
  
  for (const key of sortedKeys) {
    // Require bidirectional containment or high overlap
    const keyMatch = platformKey.includes(key) || key.includes(platformKey);
    if (keyMatch) {
      // Avoid false positives like "google ads" matching "google analytics"
      const keyWords = key.split(/\s+/);
      const platformWords = platformKey.split(/\s+/);
      
      // Check if primary distinguishing words match
      const keyHasAds = key.includes('ads') || key.includes('advertising');
      const keyHasAnalytics = key.includes('analytics') || key.includes('ga4');
      const platformHasAds = platformKey.includes('ads') || platformKey.includes('advertising');
      const platformHasAnalytics = platformKey.includes('analytics') || platformKey.includes('ga4');
      
      // Skip if one has "ads" and the other has "analytics" (they're different products)
      if ((keyHasAds && platformHasAnalytics) || (keyHasAnalytics && platformHasAds)) {
        continue;
      }
