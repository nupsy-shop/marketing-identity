...
export function getClientAssetFields(platformName, itemType = 'NAMED_INVITE') {
  ...
  // Normalize platform name for matching (remove special chars, common variations)
  const normalize = (str) => str.toLowerCase()
    .replace(/[\/\(\)\-_]/g, ' ')  // Replace special chars with spaces
    .replace(/\s+/g, ' ')          // Collapse multiple spaces
    .trim();
  
  const normalizedPlatform = normalize(platformKey);
  ...
  for (const key of Object.keys(ASSET_FIELDS_BY_PLATFORM)) {
    ...
    const platformWords = normalizedPlatform.split(/\s+/).filter(w => w.length > 2);
    
    // Count matching significant words
    const matchingWords = keyWords.filter(kw => 
      platformWords.some(pw => pw.includes(kw) || kw.includes(pw))
    );
    
    // Require at least 2 significant matching words OR one very specific match
    const hasGoodMatch = matchingWords.length >= 2 || 
      (matchingWords.length === 1 && matchingWords[0].length > 6);
    
    if (hasGoodMatch) {
      // Additional check: avoid mixing up "google ads" with "google analytics"
      const keyHasAds = key.includes('ads') && !key.includes('analytics');
      const keyHasAnalytics = key.includes('analytics') || key.includes('ga4');
      const platformHasAds = platformKey.includes('ads') && !platformKey.includes('analytics');
