...
const ACCESS_ITEM_TYPES: AccessItemTypeMetadata[] = [
  {
    ...
    description: 'Add via Business Access',
    icon: 'fas fa-handshake',
    roleTemplates: [
      { key: 'admin', label: 'Admin', description: 'Full business access' },
...
const SECURITY_CAPABILITIES: SecurityCapabilities = {
  ...
  pamRationale: 'Pinterest supports Business Access delegation and named-user invites. Shared credentials (PAM) should be used only for break-glass, legacy constraints, or client-mandated shared logins; prefer the native delegation/RBAC options.'
};

// ─── Agency Config Schemas ─────────────────────────────────────────────────────

const PartnerDelegationAgencySchema = z.object({
  businessAccountId: z.string().min(1, 'Business Account ID is required').describe('Pinterest Business Account ID'),
});
...
const CommonClientSchema = z.object({
  adAccountId: z.string().min(1, 'Ad Account ID is required').describe('Pinterest Ad Account ID'),
...
class PinterestPlugin implements PlatformPlugin {
  manifest: PluginManifest = {
    platformKey: 'pinterest',
    displayName: 'Pinterest Ads',
    category: 'Social',
    description: 'Pinterest Ads Manager',
  ...
  buildClientInstructions(context: InstructionContext): InstructionStep[] {
    ...
    switch (accessItemType) {
      case 'PARTNER_DELEGATION':
        return [
          { step: 1, title: 'Sign in to Pinterest', description: 'Go to ads.pinterest.com and sign in.', link: { url: 'https://ads.pinterest.com', label: 'Open Pinterest Ads' } },
          { step: 2, title: 'Go to Business Access', description: 'Navigate to Business Access in your settings.' },
          { step: 3, title: 'Add Partner', description: 'Add the agency as a business partner.' },
          { step: 4, title: 'Grant Access', description: `Grant "${roleTemplate}" access and confirm.` }
        ];
      case 'NAMED_INVITE':
        return [
          { step: 1, title: 'Sign in to Pinterest', description: 'Go to ads.pinterest.com and sign in.', link: { url: 'https://ads.pinterest.com', label: 'Open Pinterest Ads' } },
          { step: 2, title: 'Go to Ad Account', description: 'Navigate to your ad account settings.' },
          { step: 3, title: 'Add User', description: 'Click "Add people" and enter the agency email.' },
          { step: 4, title: 'Set Role', description: `Assign the "${roleTemplate}" role and send invitation.` }
        ];
      case 'SHARED_ACCOUNT':
        return [
          { step: 1, title: 'Provide Credentials', description: 'Enter the Pinterest account email and password.' },
