...
class TradeDeskPlugin implements PlatformPlugin, AdPlatformPlugin {
  ...
  buildClientInstructions(context: InstructionContext): InstructionStep[] {
    ...
    switch (accessItemType) {
      case 'PARTNER_DELEGATION': return [{ step: 1, title: 'Open Trade Desk', description: 'Go to thetradedesk.com', link: { url: 'https://desk.thetradedesk.com', label: 'Open Trade Desk' } }, { step: 2, title: 'Add Partner', description: `Add partner ID: ${(agencyConfig as any).partnerId}` }];
      case 'NAMED_INVITE': return [{ step: 1, title: 'Open Trade Desk', description: 'Go to thetradedesk.com' }, { step: 2, title: 'Invite User', description: `Invite agency email with "${roleTemplate}" role.` }];
  ...
  async grantAccess(params: PluginOperationParams): Promise<GrantResult> {
    ...
    return { success: false, error: `The Trade Desk does not support programmatic access granting. Use manual client instructions.`, details: { found: false } };
  }

  async verifyAccess(params: PluginOperationParams): Promise<VerifyResult> {
    const errors = validateProvisioningRequest(this.manifest, params);
    if (errors.length > 0) return { success: false, error: errors.join('; '), details: { found: false } };
    return { success: false, error: `The Trade Desk does not support programmatic access verification. Manual verification required.`, details: { found: false } };
  }

  async revokeAccess(params: PluginOperationParams): Promise<RevokeResult> {
    return { success: false, error: `The Trade Desk does not support programmatic access revocation. Remove access manually via Trade Desk settings.` };
  }
