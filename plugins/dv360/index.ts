...
class DV360Plugin implements PlatformPlugin, AdPlatformPlugin {
  readonly name = 'dv360';
  ...
  buildClientInstructions(context: InstructionContext): InstructionStep[] {
    ...
    switch (accessItemType) {
      case 'PARTNER_DELEGATION': return [{ step: 1, title: 'Open DV360', description: 'Go to displayvideo.google.com', link: { url: 'https://displayvideo.google.com', label: 'Open DV360' } }, { step: 2, title: 'Add Partner', description: `Add partner ID: ${(agencyConfig as any).partnerId}` }];
      case 'NAMED_INVITE': return [{ step: 1, title: 'Open DV360', description: 'Go to displayvideo.google.com' }, { step: 2, title: 'Invite User', description: `Invite agency email with "${roleTemplate}" role.` }];
  ...
  async grantAccess(params: PluginOperationParams): Promise<GrantResult> {
    ...
    return { success: false, error: `DV360 does not support programmatic access granting. Use manual client instructions.`, details: { found: false } };
  }

  async verifyAccess(params: PluginOperationParams): Promise<VerifyResult> {
    const errors = validateProvisioningRequest(this.manifest, params);
    if (errors.length > 0) return { success: false, error: errors.join('; '), details: { found: false } };
    return { success: false, error: `DV360 does not support programmatic access verification. Manual verification required.`, details: { found: false } };
  }

  async revokeAccess(params: PluginOperationParams): Promise<RevokeResult> {
    return { success: false, error: `DV360 does not support programmatic access revocation. Remove access manually via DV360 settings.` };
  }
